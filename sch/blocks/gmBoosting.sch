v {xschem version=3.4.8RC file_version=1.3}
G {}
K {}
V {}
S {}
F {}
E {}
T {100uA} 415 510 0 0 0.3 0.3 {}
T {0.209} 100 405 0 0 0.3 0.3 {}
T {0.56} 430 385 0 0 0.3 0.3 {}
T {0.22} 140 475 0 0 0.3 0.3 {}
T {0.95} 150 285 0 0 0.3 0.3 {}
T {0.35} -340 505 0 0 0.3 0.3 {}
T {323.66uA} -30 200 0 0 0.3 0.3 {}
T {0.72V} 40 380 0 0 0.3 0.3 {}
T {0.61
} -520 305 0 0 0.2 0.2 {}
T {0.95
} -660 340 0 0 0.2 0.2 {}
T {0.33
} -725 405 0 0 0.2 0.2 {}
T {0.62
} -965 400 0 0 0.2 0.2 {}
N 385 610 395 610 {lab=VSS}
N 385 220 395 220 {lab=VDD}
N 225 560 225 610 {lab=VSS}
N 225 220 225 270 {lab=VDD}
N 65 400 65 450 {lab=#net1}
N 65 220 225 220 {lab=VDD}
N 395 560 395 610 {lab=VSS}
N 395 220 395 270 {lab=VDD}
N -95 400 -95 450 {lab=#net1}
N -15 400 65 400 {lab=#net1}
N 225 300 225 310 {lab=#net2}
N 225 340 235 340 {lab=VDD}
N 235 220 235 340 {lab=VDD}
N 225 220 235 220 {lab=VDD}
N 385 340 395 340 {lab=VDD}
N 385 220 385 340 {lab=VDD}
N 235 220 385 220 {lab=VDD}
N 395 300 395 310 {lab=#net3}
N -95 400 -15 400 {lab=#net1}
N -15 380 -15 400 {lab=#net1}
N 65 480 65 500 {lab=#net4}
N 225 500 225 530 {lab=#net4}
N 225 450 235 450 {lab=VSS}
N 235 450 235 610 {lab=VSS}
N 225 610 235 610 {lab=VSS}
N 385 450 395 450 {lab=VSS}
N 385 450 385 610 {lab=VSS}
N 235 610 385 610 {lab=VSS}
N 65 220 65 270 {lab=VDD}
N 65 300 65 310 {lab=#net5}
N -15 310 65 310 {lab=#net5}
N -15 310 -15 320 {lab=#net5}
N -95 300 -95 310 {lab=#net5}
N -95 310 -15 310 {lab=#net5}
N -95 220 -95 270 {lab=VDD}
N -295 350 -245 350 {lab=VDD}
N -245 220 -95 220 {lab=VDD}
N -245 300 -245 320 {lab=#net6}
N -245 220 -245 270 {lab=VDD}
N -245 520 -245 530 {lab=Vb3}
N -245 560 -245 610 {lab=VSS}
N -205 520 -205 560 {lab=Vb3}
N -245 520 -205 520 {lab=Vb3}
N -245 380 -245 520 {lab=Vb3}
N -245 610 225 610 {lab=VSS}
N -95 220 65 220 {lab=VDD}
N 65 500 225 500 {lab=#net4}
N 225 480 225 500 {lab=#net4}
N 395 510 395 530 {lab=#net7}
N 395 370 395 420 {lab=Vo2}
N 225 370 225 420 {lab=Vo1}
N -205 350 -55 350 {lab=Vb0}
N -205 560 185 560 {lab=Vb3}
N -95 510 395 510 {lab=#net7}
N -95 480 -95 510 {lab=#net7}
N 395 480 395 510 {lab=#net7}
N -1595 220 -1595 280 {lab=VDD}
N -1595 320 -1595 540 {lab=VbbP}
N -1595 600 -1595 610 {lab=VSS}
N -1555 280 -1555 320 {lab=VbbP}
N -1595 320 -1555 320 {lab=VbbP}
N -1595 310 -1595 320 {lab=VbbP}
N -1555 280 -1445 280 {lab=VbbP}
N -1405 220 -1405 280 {lab=VDD}
N -1405 500 -1405 510 {lab=VbbN}
N -1365 500 -1365 540 {lab=VbbN}
N -1405 500 -1365 500 {lab=VbbN}
N -1405 310 -1405 500 {lab=VbbN}
N -1405 540 -1405 610 {lab=VSS}
N -1595 610 -1405 610 {lab=VSS}
N -1220 540 -1220 610 {lab=VSS}
N -1220 440 -1220 510 {lab=Va}
N -1220 220 -1220 400 {lab=VDD}
N -1180 400 -1180 440 {lab=Va}
N -1220 440 -1180 440 {lab=Va}
N -1220 430 -1220 440 {lab=Va}
N -1025 410 -1015 410 {lab=VSS}
N -1025 410 -1025 610 {lab=VSS}
N -1015 485 -1015 610 {lab=VSS}
N -1015 380 -975 380 {lab=Vb1}
N -975 380 -975 485 {lab=Vb1}
N -1015 305 -1015 380 {lab=Vb1}
N -1015 220 -1015 275 {lab=VDD}
N -625 330 -625 370 {lab=#net8}
N -625 220 -625 300 {lab=VDD}
N -585 300 -535 300 {lab=Vb4}
N -625 450 -535 450 {lab=Vb4}
N -625 430 -625 450 {lab=Vb4}
N -1365 540 -1260 540 {lab=VbbN}
N -1405 610 -1220 610 {lab=VSS}
N -1405 220 -1220 220 {lab=VDD}
N -975 380 -930 380 {lab=Vb1}
N -1015 440 -1015 455 {lab=#net9}
N -625 450 -625 515 {lab=Vb4}
N -535 300 -525 300 {lab=Vb4}
N -625 400 -575 400 {lab=VDD}
N -790 450 -750 450 {lab=Vb2}
N -790 430 -790 450 {lab=Vb2}
N -800 400 -790 400 {lab=VDD}
N -800 220 -800 400 {lab=VDD}
N -790 220 -790 340 {lab=VDD}
N -790 450 -790 515 {lab=Vb2}
N -790 545 -790 610 {lab=VSS}
N -750 400 -750 450 {lab=Vb2}
N -1015 220 -800 220 {lab=VDD}
N -800 220 -790 220 {lab=VDD}
N -1015 610 -790 610 {lab=VSS}
N -750 400 -665 400 {lab=Vb2}
N -750 340 -750 400 {lab=Vb2}
N -625 545 -625 610 {lab=VSS}
N -790 610 -625 610 {lab=VSS}
N -1220 610 -1025 610 {lab=VSS}
N -1025 610 -1015 610 {lab=VSS}
N -1220 220 -1015 220 {lab=VDD}
N -1595 220 -1405 220 {lab=VDD}
N -1610 220 -1595 220 {lab=VDD}
N -1610 610 -1595 610 {lab=VSS}
N -410 610 -245 610 {lab=VSS}
N -535 300 -535 450 {lab=Vb4}
N -410 220 -245 220 {lab=VDD}
N -790 220 -625 220 {lab=VDD}
N -410 540 -410 610 {lab=VSS}
N -370 310 -370 450 {lab=Vb0}
N -410 450 -370 450 {lab=Vb0}
N -410 430 -410 450 {lab=Vb0}
N -410 450 -410 520 {lab=Vb0}
N -410 220 -410 310 {lab=VDD}
N -420 400 -410 400 {lab=VDD}
N -420 220 -420 400 {lab=VDD}
N -410 340 -410 370 {lab=#net10}
N -370 310 -330 310 {lab=Vb0}
N -625 610 -410 610 {lab=VSS}
N -420 220 -410 220 {lab=VDD}
N -625 220 -420 220 {lab=VDD}
N -15 350 40 350 {lab=VDD}
C {iopin.sym} -1610 610 0 1 {name=p1 lab=VSS}
C {sg13g2_pr/sg13_lv_pmos.sym} 205 270 0 0 {name=M7
l=0.7u
w=19.8u
ng=20
m=2
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} -115 450 0 0 {name=M1
l=0.15u
w=35u
ng=70
m=4
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 85 450 0 1 {name=M2
l=0.15u
w=35u
ng=70
m=4
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} -35 350 0 0 {name=M0
l=0.3u
w=12u
ng=12
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 415 560 0 1 {name=M6
l=0.7u
w=20.2u
ng=5
m=2
model=sg13_lv_nmos
spiceprefix=X
}
C {lab_wire.sym} 185 450 0 0 {name=p9 sig_type=std_logic lab=Vb1}
C {lab_wire.sym} 435 450 0 1 {name=p10 sig_type=std_logic lab=Vb1}
C {lab_wire.sym} -55 350 0 0 {name=p17 sig_type=std_logic lab=Vb0}
C {ngspice_probe.sym} 225 515 0 0 {name=r1}
C {sg13g2_pr/sg13_lv_pmos.sym} 205 340 0 0 {name=M9
l=0.3u
w=23.2u
ng=10
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} 415 340 0 1 {name=M10
l=0.3u
w=23.2u
ng=10
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ngspice_probe.sym} -10 400 0 0 {name=r2}
C {lab_wire.sym} 185 340 0 0 {name=p8 sig_type=std_logic lab=Vb2}
C {lab_wire.sym} 435 340 0 1 {name=p11 sig_type=std_logic lab=Vb2}
C {lab_wire.sym} 185 270 0 0 {name=p12 sig_type=std_logic lab=Vb4}
C {lab_wire.sym} 435 270 0 1 {name=p18 sig_type=std_logic lab=Vb4}
C {ngspice_probe.sym} 395 310 0 0 {name=r3}
C {ngspice_probe.sym} 225 310 0 1 {name=r4}
C {sg13g2_pr/sg13_lv_pmos.sym} -115 270 0 0 {name=M3
l=0.13u
w=20u
ng=5
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {lab_wire.sym} 105 270 0 1 {name=p13 sig_type=std_logic lab=Vo1}
C {lab_wire.sym} -135 270 0 0 {name=p31 sig_type=std_logic lab=Vo2}
C {sg13g2_pr/sg13_lv_pmos.sym} 85 270 0 1 {name=M4
l=0.13u
w=20u
ng=5
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} -265 270 0 0 {name=M5
l=0.13u
w=20u
ng=5
m=2
model=sg13_lv_pmos
spiceprefix=X
}
C {lab_wire.sym} -295 350 0 0 {name=p34 sig_type=std_logic lab=VDD}
C {lab_wire.sym} -285 270 0 0 {name=p40 sig_type=std_logic lab=Va}
C {ngspice_probe.sym} -180 350 0 0 {name=r5}
C {sg13g2_pr/sg13_lv_nmos.sym} -225 560 0 1 {name=M29
l=0.7u
w=32u
ng=4
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {ngspice_probe.sym} -210 520 0 0 {name=r14}
C {lab_wire.sym} 125 560 0 1 {name=p41 sig_type=std_logic lab=Vb3}
C {lab_wire.sym} 435 560 0 1 {name=p42 sig_type=std_logic lab=Vb3}
C {sg13g2_pr/sg13_lv_pmos.sym} -225 350 0 1 {name=M30
l=0.3u
w=12u
ng=12
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 205 560 0 0 {name=M31
l=0.7u
w=20.2u
ng=5
m=2
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 205 450 0 0 {name=M32
l=0.6u
w=38.7u
ng=5
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {opin.sym} 225 395 0 1 {name=p2 lab=Vo1}
C {opin.sym} 395 395 0 0 {name=p5 lab=Vo2}
C {ipin.sym} -135 450 0 0 {name=p3 lab=Vi+}
C {ipin.sym} 105 450 0 1 {name=p15 lab=Vi-}
C {iopin.sym} -1610 220 0 1 {name=p14 lab=VDD}
C {isource.sym} -1595 570 0 0 {name=I3 value=50u}
C {sg13g2_pr/sg13_lv_pmos.sym} -1575 280 0 1 {name=M21
l=1u
w=18u
ng=18
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ngspice_probe.sym} -1555 280 0 0 {name=r6}
C {lab_wire.sym} -1555 320 0 1 {name=p35 sig_type=std_logic lab=VbbP}
C {sg13g2_pr/sg13_lv_pmos.sym} -1425 280 0 0 {name=M24
l=1u
w=18u
ng=18
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} -1385 540 0 1 {name=M25
l=1u
w=15u
ng=2
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} -1240 540 0 0 {name=M17
l=1u
w=15u
ng=2
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {lab_wire.sym} -1335 540 0 1 {name=p38 sig_type=std_logic lab=VbbN}
C {sg13g2_pr/sg13_lv_pmos.sym} -1200 400 0 1 {name=M18
l=1u
w=13u
ng=13
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {lab_wire.sym} -1180 410 0 1 {name=p4 sig_type=std_logic lab=Va}
C {sg13g2_pr/sg13_lv_nmos.sym} -995 410 0 1 {name=M19
l=0.7u
w=13u
ng=2
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} -995 485 0 1 {name=M20
l=0.7u
w=2.4u
ng=1
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {ngspice_probe.sym} -1015 455 0 0 {name=r7}
C {lab_wire.sym} -930 380 0 1 {name=p30 sig_type=std_logic lab=Vb1}
C {sg13g2_pr/sg13_lv_pmos.sym} -1035 275 0 0 {name=M22
l=1u
w=18u
ng=18
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {lab_wire.sym} -1055 275 0 0 {name=p26 sig_type=std_logic lab=VbbP}
C {sg13g2_pr/sg13_lv_pmos.sym} -645 400 0 0 {name=M26
l=0.3u
w=9u
ng=9
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} -605 300 0 1 {name=M38
l=0.7u
w=15.5u
ng=15
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ngspice_probe.sym} -625 360 0 0 {name=r9}
C {lab_wire.sym} -525 300 0 1 {name=p32 sig_type=std_logic lab=Vb4}
C {lab_wire.sym} -700 400 0 0 {name=p6 sig_type=std_logic lab=Vb2}
C {sg13g2_pr/sg13_lv_nmos.sym} -645 545 0 0 {name=M39
l=1u
w=15u
ng=2
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {lab_wire.sym} -665 545 0 0 {name=p36 sig_type=std_logic lab=VbbN}
C {lab_wire.sym} -575 400 0 1 {name=p39 sig_type=std_logic lab=VDD}
C {sg13g2_pr/sg13_lv_pmos.sym} -770 340 0 1 {name=M40
l=0.7u
w=4.5u
ng=1
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} -770 400 0 1 {name=M41
l=0.7u
w=15u
ng=15
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ngspice_probe.sym} -790 370 0 1 {name=r8}
C {sg13g2_pr/sg13_lv_nmos.sym} -810 545 0 0 {name=M42
l=1u
w=15u
ng=2
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {lab_wire.sym} -830 545 0 0 {name=p24 sig_type=std_logic lab=VbbN}
C {sg13g2_pr/sg13_lv_pmos.sym} 415 270 0 1 {name=M8
l=0.7u
w=19.8u
ng=20
m=2
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} 415 450 0 1 {name=M11
l=0.6u
w=38.7u
ng=5
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_nmos.sym} -430 545 0 0 {name=M12
l=1u
w=15u
ng=2
m=1
model=sg13_lv_nmos
spiceprefix=X
}
C {lab_wire.sym} -450 545 0 0 {name=p7 sig_type=std_logic lab=VbbN}
C {sg13g2_pr/sg13_lv_pmos.sym} -390 310 0 1 {name=M13
l=0.7u
w=3.92u
ng=2
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {sg13g2_pr/sg13_lv_pmos.sym} -390 400 0 1 {name=M14
l=0.15u
w=55u
ng=55
m=1
model=sg13_lv_pmos
spiceprefix=X
}
C {ngspice_probe.sym} -410 360 0 0 {name=r10}
C {ngspice_probe.sym} -370 310 0 0 {name=r11}
C {lab_wire.sym} -330 310 0 1 {name=p21 sig_type=std_logic lab=Vb0}
C {lab_wire.sym} 40 350 0 1 {name=p16 sig_type=std_logic lab=VDD}
